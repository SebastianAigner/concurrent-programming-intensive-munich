...
class ConceptuallyInfiniteArray {
    ...
    private tailrec fun findSegmentWithId(id: Long): Segment {
...
//            println("looking for $id in ${curr.id}, has ${curr.next}")
            ...
            if (next == null) {
                // we're at the tail
                if(id == curr.id + 1) {
                    // if we were to create a new segment here, it would ACTUALLY be the desired segment.
                    // so we do that.
                    ...
                    // there is a segment missing, but is it not _our_ segment.
                    // we need to create a number of additional intermediate segments.
                    curr.next.compareAndSet(null, Segment(curr.id + 1))
                    return findSegmentWithId(id)
                }
            } else {
